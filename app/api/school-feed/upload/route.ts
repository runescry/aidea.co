...
import { extractEventsFromUpload, isSupportedEventUpload, type ExtractedEvent } from '@/lib/documents/extract-event';
import { createCalendarEvent } from '@/lib/nango/calendar';
import { hasCalendarConnection } from '@/lib/nango/connections';
...
async function createEvent(
  ...
  profiles: ReturnType<typeof loadSchoolProfiles>,
  timeZone: string,
...
/** Upload a flyer/form/notice, extract its events, and add each to Google Calendar. */
export async function POST(req: NextRequest) {
  ...
  try {
    extracted = await extractEventsFromUpload({
      bytes,
      mimeType: file.type,
      filename: file.name,
  ...
  for (const event of extracted) {
    results.push(await createEvent(event, profiles, timeZone));
  }

  const anySucceeded = results.some(r => r.ok);
  if (anySucceeded) {
    // Best-effort — pull the new events straight into the school feed instead of waiting for
    // the next hourly cron. A sync failure here doesn't undo the calendar writes, so it's
    // swallowed.
    await syncSchoolCalendar().catch(() => undefined);
  }

  return NextResponse.json(
    { ok: anySucceeded, events: results },
    { status: anySucceeded ? 200 : 502 },
  );
}

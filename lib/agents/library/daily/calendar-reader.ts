...
STEP 5a: Bucket the rest of the week.
From the full events list (not just todayEvents/tomorrowEvents), take every event whose
date is after tomorrow's date — i.e. day 3 through day 6 of the range. For each, keep only
{ date, title, notes } — notes should name the child if the event or attendees clearly
belong to one of family.children, otherwise leave notes blank. This is best-effort context
for a "rest of the week" glance, not a full schedule — skip internal/solo blocks with no
family or logistics relevance. Omit weekAhead entirely if there's nothing worth surfacing.
...
weekAhead: from step 5a. Optional — omit the key entirely rather than writing an empty array if there's nothing to add.

...
import { activePeopleCount, peopleContactsForEditor } from '@/lib/profile/people';
...
export function getOnboardingProgress(kb: KnowledgeBase): { completed: number; total: number } {
  const checks = [
    ...
    Boolean(
      peopleContactsForEditor(kb, 'contact').length > 0
      || (kb.work?.keyStakeholders?.length ?? 0) > 0,
    ),
    ...
    Boolean(activePeopleCount(kb) > 0),

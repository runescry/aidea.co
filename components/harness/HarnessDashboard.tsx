...
type MainTab = 'run' | 'chat' | 'queue' | 'profile';
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100 flex flex-col">
      ...
      <HumanInputOverlay
        ...
        onSubmit={(_requestId, _answer) => clearPendingInput()}
      />
      ...
      <header className="border-b border-gray-800 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          {mainTab === 'run' && <StatusBadge status={state.status} />}
        </div>
      </header>
      ...
      {mainTab === 'run' && (
        <>
          ...
          <div className="border-b border-gray-800 px-6 py-4 space-y-4">
            ...
            {/* Daily OS — no config needed */}
            {!hasContent && entity === 'daily' && (
              <div className="max-w-3xl">
                <p className="text-xs text-gray-500 mb-3">
                  Runs inbox triage, calendar, health brief, news curation, and work prep. Set your profile in the Profile tab first.
                </p>
                <button
                  onClick={handleStart}
                  className="px-5 py-2 bg-blue-600 hover:bg-blue-500 text-white font-medium rounded text-sm transition-colors"

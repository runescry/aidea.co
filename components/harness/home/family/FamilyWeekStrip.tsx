...
import { childDotClass, childTextClass } from './childColors';
          ...
          {selectedDay.events.length > 0 ? (
            <ul className="space-y-1.5">
              {selectedDay.events.map((event, i) => {
                ...
                const titleNamesChild = child && event.title.toLowerCase().includes(child.name.split(/\s+/)[0]!.toLowerCase());
                return (
                  <li key={i} className="text-[14px] text-foreground-muted leading-snug">
                    {child && !titleNamesChild && (
                      <span className={`font-medium ${childTextClass(child.colorIndex)}`}>{child.name}{' '}</span>
                    )}
                    {event.title}
                  </li>
                );
